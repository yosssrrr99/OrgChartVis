...
export class RemunerationComponent {
  ...
  updateSalary(event: any, employee: any) {
    ...
    if (!isNaN(percentage)) {
      const newSalary = this.initialSalary + (this.initialSalary * percentage / 100);
      const salaryDifference = newSalary - employee.mtsal;
  
      employee.mtsal = newSalary;
  
      // Update the budgetAnnuel1
      this.budgetAnnuel1 += salaryDifference;
     
  
      // Update gab1
      this.updateGab1();
      this.percentageChangeTab1 =  this.calculerPourcentageBudgetDep(this.budgetGlobal1, this.gab1);
      this.percentageChangeResTab1 = 100 - this.percentageChangeTab1;
    }
}
  initializeEmployees(employees: any[]) {
    employees.forEach(employee => {
      this.initialSalary = employee.mtsal; // Store the initial salary
    });
  }
  ...
  this.employeeService.getEmployeesByDepartment(organisationId).subscribe((response: EmployeeAndTotalSalaryResponse) => {
    if (listId === 'tab1') {
      ...
      this.oldBuget2 = this.budgetGlobal2 - this.budgetAnnuel2;
         this.initializeEmployees(this.tab2Data); 
    }
  });
  }
